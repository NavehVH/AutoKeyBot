...
namespace AutoKeyBot.Applications
{
    public partial class Potions : Form
    {
        ...
        public Potions(String potionType)
        {
            InitializeComponent();
        ...
        private void Potions_Load(object sender, EventArgs e)
        {
            ...
            EventTitle.Text = PotionType.ToUpper() + " Event:";
            ActiveText.Text = "Active event when " + PotionType.ToUpper() + " is:";
            ...
            if (Program.EventId != -1)
            {
                ...
                if (!eventEdit.Bigger1)
                    smallerRadio.Checked = true;
                FromPotion.Text = eventEdit.From1.ToString();
            }
        ...
        private void EventFormClosing(object sender, FormClosingEventArgs e)
        {
            if (Program.EventId != -1)
            {
                ...
                if (!smallerRadio.Checked)
                {
                    MessageBox.Show("You didn't choose when the event should happened.");
                    e.Cancel = true;
                    return;
                }
                if (FromPotion.Text == "")
                {
                    MessageBox.Show("You didn't choose amount of " + PotionType.ToUpper() + " to active the event.");
                    e.Cancel = true;
                    return;
                }
                if (int.TryParse(FromPotion.Text, out int n) == false)
                {
                    MessageBox.Show("You need to enter a number as " + PotionType.ToUpper() + ".");
                    e.Cancel = true;
                    return;
                }

                if (int.Parse(FromPotion.Text) > 100 || int.Parse(FromPotion.Text) < 0)
                {
                    MessageBox.Show("Your " + PotionType.ToUpper() + " needs to between 1-100%");
                    e.Cancel = true;
                    return;
                }
        ...
        private void SaveEventData(Event e)
        {
            ...
            e.EventType = PotionType;
            e.From1 = int.Parse(FromPotion.Text);
            if (smallerRadio.Checked)
                e.Bigger1 = false;
        }
        ...
        private void AddKey_Click(object sender, EventArgs e)
        {
            if (Key.Text != "")
            {
                ...
                if (Program.EventId == -1)
                {
                    c.NonQuery("INSERT INTO bot ([keystroke], [timer], [position], [botId], [eventId], [eventType]) VALUES ('" + Key.Text + "', " + spamTime + ", " + maxPosition + ", " + Program.BotId + ", -1, '" + PotionType + "')");
                }
                else
                    c.NonQuery("INSERT INTO bot ([keystroke], [timer], [position], [botId], [eventId], [eventType]) VALUES ('" + Key.Text + "', " + spamTime + ", " + maxPosition + ", " + Program.BotId + ", " + Program.EventId + ", '" + PotionType + "')");
